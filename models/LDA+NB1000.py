...
data_file = 'one-hot-encoding1000.txt'
X, y = load_data(data_file)
scaler = StandardScaler()
X = scaler.fit_transform(X)

# Dimensionality reduction with LDA
y_labels = np.argmax(y, axis=1)  # Convert one-hot labels to indices
lda = LDA(n_components=min(X.shape[1], y.shape[1] - 1))  # Number of components for LDA
...
def plot_confusion_matrix(y_test, y_pred, malware_names, output_file="confusion_matrix_LDA+NB.png"):
...
# Visualizing the loss and accuracy (in this case, simulating similar metrics to neural network graphs)
plt.figure(figsize=(14, 6))

# Calculate and display metrics by class
def calculate_metrics(y_test, y_pred, malware_names, output_file="metrics_results_LDA+NB.csv", image_output="metrics_per_class_LDA+NB.png"):
    """
