...
archivo_datos = 'one-hot-encoding1000.txt'
X, y = cargar_datos(archivo_datos)
scaler = StandardScaler()
X = scaler.fit_transform(X)

# Reducir dimensionalidad con LDA
y_labels = np.argmax(y, axis=1)  # Convertir etiquetas one-hot a índices
lda = LDA(n_components=min(X.shape[1], y.shape[1] - 1))  # Número de componentes para LDA
X_lda = lda.fit_transform(X, y_labels)


# Dividir los datos en conjunto de entrenamiento y prueba
X_train, X_test, y_train, y_test = train_test_split(X_lda, y_labels, test_size=0.2, random_state=42)
...
def graficar_matriz_confusion(y_test, y_pred, nombres_malware, archivo_salida="matriz_confusion_LDA+NB_casero.png"):
...
# Visualizar la pérdida y precisión (en este caso, simulamos métricas similares a las gráficas de redes)
plt.figure(figsize=(14, 6))

# Calcular y visualizar métricas por clase
def calcular_métricas(y_test, y_pred, nombres_malware, archivo_salida="resultados_métricas_LDA+NB.csv", imagen_salida="metricas_por_clase_LDA+NB.png"):
    """
