...
_, fitting_measure = measure_call(lambda: xgb_model.fit(X_train, y_train))
...
if n_classes == 2:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("roc_curve_LDA+XGB1000.png")
    print("ROC curve saved as 'roc_curve_LDA+XGB1000.png'")
    plt.close()
    ...
    plt.figure(figsize=(8, 6))
    plt.plot(recall, precision, label=f"XGBoost (PR-AUC = {pr_auc:.3f})")
    plt.axhline(
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("pr_curve_LDA+XGB1000.png")
    print("Precision-Recall curve saved as 'pr_curve_LDA+XGB1000.png'")
    plt.close()
...
auc_results.to_csv("auc_results_LDA+XGB1000.csv", index=False)
print("AUC results saved in 'auc_results_LDA+XGB1000.csv'")
...
def predict_end_to_end(X_batch_raw):
    ...
    return xgb_model.predict(X_batch)

inference_metrics = benchmark_inference(predict_end_to_end, X_test_raw)
artifact_files = ['modelo_scaler.pkl', 'modelo_LDA3.pkl', 'modelo_XGB.pkl']
model_size_bytes = total_file_size_bytes(artifact_files)

deployment_metrics = base_run_metadata('LDA+XGB', 'VM-packer', 42)
...
save_metrics(deployment_metrics, csv_path='deployment_metrics_vm_packer.csv', json_path='deployment_metrics_LDA_XGB_vm_packer.json')
