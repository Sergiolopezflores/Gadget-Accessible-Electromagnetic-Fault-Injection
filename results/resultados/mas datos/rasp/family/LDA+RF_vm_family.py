...
_, fitting_measure = measure_call(lambda: rf_model.fit(X_train, y_train))
...
def predict_end_to_end(X_batch_raw):
    ...
    return rf_model.predict(X_batch)

inference_metrics = benchmark_inference(predict_end_to_end, X_test_raw)
artifact_files = ['modelo_scaler.pkl', 'modelo_LDA3.pkl', 'modelo_RF.pkl']
model_size_bytes = total_file_size_bytes(artifact_files)

deployment_metrics = base_run_metadata('LDA+RF', 'VM-family', 42)
...
save_metrics(deployment_metrics, csv_path='deployment_metrics_vm_family.csv', json_path='deployment_metrics_LDA_RF_vm_family.json')
